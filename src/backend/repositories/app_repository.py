...
class AppRepository:
    ...
    def update_game_metadata(self, app_id, icon_path, genre, year, rating=None):
        """Обновляет или добавляет все метаданные для игры в таблице game_metadata."""
        if genre:
            genre = genre.strip()
            if not genre:
                genre = None
        print(f"Updating metadata for app_id={app_id}: icon_path={icon_path}, genre={genre}, year={year}, rating={rating}")
        query = """
        ...
        """
        self.db.cursor.execute(query, (app_id, icon_path, genre, year, rating))
        self.db.connection.commit()


    def get_games_list_with_rating(self):
        """Возвращает список всех игр с названием, датами первого и последнего запуска, общим временем, количеством сессий, жанром, годом, путём к иконке и рейтингом."""
        query = """
            SELECT
                COALESCE(a.alias, a.name) as name,
                MIN(s.start_time) as first_played,
    ...
    def update_game_rating(self, app_id, rating):
        """Обновляет только рейтинг игры в таблице game_metadata."""
        print(f"Updating rating for app_id={app_id} to {rating}")
        query = """
            ...
            DO UPDATE SET
                rating = EXCLUDED.rating
        """
        self.db.cursor.execute(query, (app_id, rating))
        self.db.connection.commit()
